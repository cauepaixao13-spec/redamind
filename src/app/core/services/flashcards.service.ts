...
export class FlashcardsService {
  ...
  constructor(private storage: StorageService, private auth: AuthService) {
    // IMPORTANTE — por que carregamos de forma síncrona aqui, e não só dentro do effect():
    // effect() sempre roda de forma ASSÍNCRONA (agendado como microtask), mesmo quando
    // registrado dentro do construtor. Isso significa que se um COMPONENTE injeta este
    // serviço e lê `cards$()`/`categories()` dentro do seu próprio `ngOnInit()` (que roda
    // de forma SÍNCRONA, no mesmo turno de execução), ele veria os arrays ainda vazios —
    // porque o effect() abaixo ainda não teve chance de rodar. Carregando aqui, de forma
    // síncrona, com o valor ATUAL de `auth.currentUser()`, garantimos que os dados já
    // estejam prontos assim que qualquer outro serviço/componente for construído em seguida
    // (a ordem de resolução do Angular DI garante que o AuthService — e sua sessão restaurada
    // de forma síncrona — já existe antes deste construtor rodar).
    this.loadForUser(this.auth.currentUser()?.id ?? null);

    // O effect() continua existindo pra reagir a mudanças de usuário DEPOIS do carregamento
    // inicial (login/logout durante a mesma sessão do navegador, sem recarregar a página).
    effect(() => {
      const userId = this.auth.currentUser()?.id ?? null;
      if (userId === this.lastLoadedUserId) return; // evita recarregar 2x no boot inicial
  ...
  private loadForUser(userId: string | null) {
    ...
    try {
      ...
      const mergedCards = [...predefined, ...(Array.isArray(customCards) ? customCards : [])].map(c => {
      ...
      // Nunca deixamos isso falhar em silêncio: se der erro (dado corrompido no
      // LocalStorage, storage indisponível etc.), ao menos os 50 cards predefinidos
      // continuam disponíveis, e o erro fica visível no console pra investigar.
      console.error('[FlashcardsService] Falha ao carregar flashcards do usuário:', err);
      this.baseCards.set(this.getPredefinedCards());
      this.sessions.set([]);
    }
  }
